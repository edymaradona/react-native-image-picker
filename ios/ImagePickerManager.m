...
/*
 * Synthesizing properties on our Obj-C classes allows us to access properties
 * using the _property notation instead of self.property
 *
 * EG:
 * self.audioController OR _audioController will access the same property now.
 */
...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary *)info
{
    dispatch_block_t dismissCompletionBlock = ^{
        ...
        // fire the event again to update the processing status
        [_bridge.eventDispatcher sendAppEventWithName:@"imagePickerStatus" body:@[@{@"isProcessing": @NO}]];
        
        self.callback(@[response]);
    };
    dispatch_async(dispatch_get_main_queue(), ^{
        // fire the event to let the front end know that the photo is being processed
        [_bridge.eventDispatcher sendAppEventWithName:@"imagePickerStatus" body:@[@{@"isProcessing": @YES}]];
